...
const scoreButton = document.getElementById('score');           // ← nuevo botón
...
// Base de datos PouchDB (se guarda en el navegador del usuario)
let db = new PouchDB('asteroids_db');

// Estadísticas que guardaremos en la base de datos
let stats = {
    _id: 'playerStats',
    gamesPlayed: 0,
    totalPoints: 0
...
// Cargar o crear las estadísticas al iniciar
db.get('playerStats').then(doc => {
    stats = doc;
}).catch(err => {
    // Si no existe, lo creamos con valores en 0
    if (err.status === 404) {
        db.put(stats);
    }
});
